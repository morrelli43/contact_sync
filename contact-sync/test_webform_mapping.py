...
class TestWebFormMapping(unittest.TestCase):
    ...
    def test_external_form_fields(self):
        """Test mapping from external booknow.html form field names."""
        test_data = {
            'first_name': 'Jane',
            'surname': 'Doe',
            'number': '0412345678',
            'address_line_1': '23 Batman Street',
        ...
        self.assertEqual(contact.phone, '0412345678')
        self.assertEqual(contact.notes, 'General Service')
        self.assertEqual(contact.extra_fields.get('escooter1'), 'Segway Max G30')
        
        # Verify address components are correctly separated
        self.assertEqual(len(contact.addresses), 1)
        ...
        self.assertEqual(contact.addresses[0]['postal_code'], '3003')
